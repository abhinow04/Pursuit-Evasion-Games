...
class EV3_MocapNode(Node):
    def __init__(self):
        ...
        self.pursuer1_publisher = self.create_publisher(PoseStamped, "pursuer1/pose", 10)
        self.pursuer2_publisher = self.create_publisher(PoseStamped, "pursuer2/pose", 10)
    ...
    def timer_callback(self):
        ...
        for name, obj in self.mc.rigidBodies.items():
            ...
            print(position)
            if str(name) == "pursuer1":
                self.pursuer1_publisher.publish(msg)
                self.get_logger().info('Publishing: "{}":"{}" "{}"'.format(msg.header.frame_id, msg.pose.position, msg.pose.orientation))
                print(msg)
            elif str(name) == "pursuer2":
                self.pursuer2_publisher.publish(msg)
                self.get_logger().info('Publishing: "{}":"{}" "{}"'.format(msg.header.frame_id, msg.pose.position, msg.pose.orientation))
