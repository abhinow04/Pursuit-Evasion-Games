...
class assignment:
    def __init__(self,pursuer_position,evader_position,target_position,speed,pur_sp,eva_sp):
        ...
        self.pursuer_velocities = []
        self.evader_velocities = []
    ...
    def linporgram(self):
        ...
        for i in range(self.m):
            print(f'Pursuer {self.evaders[i].pursuer[0]} has been assigned to chase the evader')
            self.assigned_list.append(self.evaders[i].pursuer[0])
            
        self.assigned_list = list(set(self.assigned_list))
    ...
    def plot_contin(self):
        ...
        while not done:
            ...
            for j in range(self.n):
                ...
                plt.scatter(self.pursuers[j].position[0], self.pursuers[j].position[1], color=clr[j], marker='o', s=100)
            ...
            plt.pause(0.0001)
            print(f"Step {t}: Evader statuses = {[e.status for e in self.evaders]}")
    ...
    def check_win(self):
        ...
        if mask.any():
            ...
            for x in self.assigned_list:
                for pursuer in self.pursuers:
                    if pursuer.index == x:
                        print(x)
                        self.pursuer_velocities.append(pursuer.return_velocity(self.evaders[pursuer.evader], self.B[pursuer.evader, pursuer.index], self.tolerance))

            for evader in self.evaders:
                self.evader_velocities.append(evader.return_velocity(self.pursuers[evader.pursuer], self.B[evader.index, evader.pursuer], self.tolerance))

            pursuer_wheel_vels = [gogoal.OmniRobot(np.array([p.position[0], p.position[1]]), 0).move_to_target(pv) for p, pv in zip(self.pursuers, self.pursuer_velocities)]
            evader_wheel_vels = [gogoal.OmniRobot(np.array([e.position[0], e.position[1]]), 0).move_to_target(ev) for e, ev in zip(self.evaders, self.evader_velocities)]

            import ev3_gotogoal
            ev3_gotogoal.send_wheel_velocities(pursuer_wheel_vels, evader_wheel_vels , self.assigned_list)
    ...
    def step(self): 
        ...
        for i, evader in enumerate(self.evaders):
            if evader.status == 0:
                ...
                if j.size > 0:
                    evader.updatePos(evader.position + self.timestep * evader.return_velocity(self.pursuers[j[0]], self.B[i, j[0]], self.tolerance))

        for j, pursuer in enumerate(self.pursuers):
            if pursuer.status == -1:  
                i = np.where(self.x[:, j] == 1)[0]
                if i.size > 0:
                    pursuer.updatePos(pursuer.position + self.timestep * pursuer.return_velocity(self.evaders[i[0]], self.B[i[0], j], self.tolerance))
